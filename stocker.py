...
class Stocker():
    ...
    def __init__(self, ticker):
        ...
        self.min_price_date = self.stock[self.stock['y']
                                         == self.min_price]['Date']
        self.min_price_date = self.min_price_date[self.min_price_date.index[0]]
        self.max_price_date = self.stock[self.stock['y']
                                         == self.max_price]['Date']
        ...
        self.most_recent_price = float(
            self.stock.loc[self.stock.index[-1], 'y'])
    ...
    def plot_stock(self, start_date=None, end_date=None, stats=['Adj. Close'], plot_type='basic'):
        ...
        for i, stat in enumerate(stats):
            ...
            print('Maximum {} = {:.2f} on {}.'.format(
                stat, stat_max, date_stat_max))
            print('Minimum {} = {:.2f} on {}.'.format(
                stat, stat_min, date_stat_min))
            print('Current {} = {:.2f} on {}.\n'.format(
            ...
            # Stat y-axis
                ...
                plt.style.use('fivethirtyeight')
                plt.plot(stock_plot['Date'], stock_plot[stat],
                         color=colors[i], linewidth=3, label=stat, alpha=0.8)
                plt.xlabel('Date')
                plt.ylabel('US $')
                plt.title('%s Stock History' % self.symbol)
                plt.legend(prop={'size': 10})
    ...
    def changepoint_prior_analysis(self, changepoint_priors=[0.001, 0.05, 0.1, 0.2], colors=['b', 'r', 'grey', 'gold']):
        ...
        for prior in changepoint_priors:
            ...
            ax.fill_between(predictions['ds'].dt.to_pydatetime(), predictions['%.3f_yhat_upper' % prior],
                            predictions['%.3f_yhat_lower' %
                                        prior], facecolor=color_dict[prior],
    ...
    def evaluate_prediction(self, start_date=None, end_date=None, nshares=None):
        ...
        test = self.stock[(self.stock['Date'] >= start_date)
                          & (self.stock['Date'] <= end_date)]
        ...
        test['correct'] = (np.sign(test['pred_diff'][1:]) ==
                           np.sign(test['real_diff'][1:])) * 1
    ...
    def changepoint_date_analysis(self, search=None):
        ...
        if not search:
            ...
            plt.plot(future['ds'], future['yhat'],
                     color='navy', linewidth=2.0, label='Modeled')
        ...
        if search:
            date_range = ['%s %s' %
                          (str(min(train['Date'])), str(max(train['Date'])))]

            # Get the Google Trends for specified terms and join to training dataframe
            trends, related_queries = self.retrieve_google_trends(
                search, date_range)
            ...
            plt.plot(train['ds'], train['freq_norm'],
                     color='goldenrod', label='Search Frequency')
    ...
    def changepoint_prior_validation(self, start_date=None, end_date=None, changepoint_priors=[0.001, 0.05, 0.1, 0.2]):
        ...
        for i, prior in enumerate(changepoint_priors):
            ...
            train_results = pd.merge(
                train, future[['ds', 'yhat', 'yhat_upper', 'yhat_lower']], on='ds', how='inner')
            ...
            test_results = pd.merge(
                test, future[['ds', 'yhat', 'yhat_upper', 'yhat_lower']], on='ds', how='inner')
        ...
        plt.plot(results['cps'], results['train_err'],
                 'bo-', ms=8, label='Train Error')
        plt.plot(results['cps'], results['test_err'],
                 'r*-', ms=8, label='Test Error')
        ...
        plt.plot(results['cps'], results['train_range'],
                 'bo-', ms=8, label='Train Range')
        plt.plot(results['cps'], results['test_range'],
                 'r*-', ms=8, label='Test Range')
        plt.xlabel('Changepoint Prior Scale')
